...
url_fm = "https://ws.audioscrobbler.com/2.0/?method=artist.search&artist=lil&api_key=c778f946c09ad20b2014470344971fd3&format=json"
limit = {'limit' : 50}
response = requests.get(url_fm, params = limit)
data_fm = response.json()


# In[96]:


data_fm['results']['artistmatches']['artist'][0]

for x in data_fm['results']['artistmatches']['artist']:
    print(x['name'], "has", x['listeners'] ,"listeners")
...
listenerlist = [x['listeners'] for x in data_fm['results']['artistmatches']['artist'] ]

for i in range(0, len(listenerlist)):
    listenerlist[i] = int(listenerlist[i])
    
print(sum(listenerlist))
...
data_fm['results']['artistmatches']['artist'][0]['image'][-2]
...
data_fm['results']['artistmatches']['artist'][4]['mbid']
...
url_mbid = "https://ws.audioscrobbler.com/2.0/?method=artist.getinfo&api_key=c778f946c09ad20b2014470344971fd3&format=json"
mbid = {'mbid' : 'a95384b1-6aec-468c-ae0d-8c6daf87c4c2'}
response = requests.get(url_mbid, params = mbid)
...
print(data_mbid['artist']['bio']['summary'])
...
for x in data_mbid['artist']['tags']['tag']:
    print(x['name'])
...
mbidlist = [x['mbid'] for x in data_fm['results']['artistmatches']['artist'] if x['mbid'] != ""]
print(mbidlist)
...
url_mbids = "https://ws.audioscrobbler.com/2.0/?method=artist.getinfo&api_key=c778f946c09ad20b2014470344971fd3&format=json"
for x in mbidlist:
    response = requests.get(url_mbids, params = x)
data_mbids = response.json()

for x in mbidlist:
    print(f"https://ws.audioscrobbler.com/2.0/?method=artist.getinfo&api_key=c778f946c09ad20b2014470344971fd3&format=json&mbid={x}")
...
mbidslist = [f"https://ws.audioscrobbler.com/2.0/?method=artist.getinfo&api_key=c778f946c09ad20b2014470344971fd3&format=json&mbid={x}" for x in mbidlist]

mbidslist[:3]

for x in mbidslist[:3]:
    url_mbids = x
    response = requests.get(url_mbids)
    data_mbids = response.json()
    print(data_mbids['artist']['name'])
...
for x in mbidslist[:3]:
    ...
    data_mbids = response.json()
    print(data_mbids['artist']['name'])
    for y in data_mbids['artist']['tags']['tag']:
        print(y['name'])
...
raplist = ['hip hop', 'rap', 'Hip-Hop']

for x in mbidslist[:10]:
    url_mbids = x
    response = requests.get(url_mbids)
    data_mbids = response.json()
    print("Artist:", data_mbids['artist']['name'], '-------')
    rapper = 0 
    for y in data_mbids['artist']['tags']['tag']:
        if y['name'] in raplist:
            rapper = rapper + 1
    if rapper == 0:
        print("not a rapper")
    elif rapper > 0:
...
for x in mbidslist:
    ...
    for y in data_mbids['artist']['tags']['tag']:
        if y['name'] in raplist:
            rapper = rapper + 1
    if rapper > 0:
            rappercount = rappercount + 1 
                
print(round(rappercount/len(mbidslist)*100), "%")
